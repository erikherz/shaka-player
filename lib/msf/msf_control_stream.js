...
shaka.msf.ControlStreamDecoder = class {
  ...
  async messageType_() {
    ...
    switch (type) {
      ...
      case shaka.msf.Utils.MessageTypeId.REQUESTS_BLOCKED:
        ...
        break;
      default:
        shaka.log.warning(`Skipping unknown message type: 0x${type.toString(16)} (${messageLength} bytes)`);
        await this.reader_.read(messageLength);
        return null;
    }
  ...
  async publishDone_() {
    ...
    // Read stream count before reason (matches relay wire order)
